...
void init_enigme1(enigme1 *e1 )
{
 ...
 e1->police=TTF_OpenFont("texxte.ttf",1);
e1->policequestion=TTF_OpenFont("texxte.ttf",1);
...
SDL_Color couleurvert={140,100,50};  
SDL_Color couleurnoir={10,10,10};
SDL_Color couleurrouge={37,80,145};
...
e1->policebravo=TTF_OpenFont("texxte.ttf",1);
e1->textebravo=TTF_RenderText_Blended(e1->policebravo,"WELL DONE !",couleurvert);   //initialistion de tout ce qui concerne affichge reponse juste//
e1->posbravo.x=470;
e1->posbravo.y=520;
...
e1->policeincorrecte=TTF_OpenFont("texxte.ttf",1);
e1->texteincorrecte=TTF_RenderText_Blended(e1->policeincorrecte,"INCORRECT GOOD LUCK NEXT TIME !",couleurrouge);  //initialistion de tout ce qui concerne affichge reponse fausse//
e1->posincorrecte.x=418;
e1->posincorrecte.y=520;
